...
public class CommentController {
    ...
    // 댓글 등록
    @PostMapping("/{boardNo}")
    public ResponseEntity<?> create(Comment comment, @PathVariable Integer boardNo) {
    ...
    public ResponseEntity<?> update(Comment comment, @PathVariable Integer boardNo, @PathVariable Integer commentNo) {
    ...
    public ResponseEntity<?> destroy(@PathVariable Integer commentNo, @PathVariable Integer boardNo) {
        ...
        try {
            ...
            if( result > 0 )
                ...
                return new ResponseEntity<>("댓글 삭제 실패", HttpStatus.OK);
        } catch (Exception e) {
            log.error(null, e);
            return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
        }
